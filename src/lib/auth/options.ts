...
function buildProviders(): NextAuthOptions["providers"] {
    ...
    const linkByEmail = Boolean(prisma)
    ...
    if (googleId && googleSecret) {
        list.push(
            GoogleProvider({
                ...
                // Same verified email → one User row (shared blueprint ownership across Google / other OAuth).
                allowDangerousEmailAccountLinking: linkByEmail,
            })
        )
    }
