...
function BranchTabBar({ active, onChange }: TabBarProps) {
    return (
        <div className="flex gap-1 rounded-xl p-1 border border-white/[0.06]"
...
export function SkillTreePlanner() {
    ...
    return (
        <div className="flex flex-col gap-6 overflow-x-hidden">
            ...
            {/* ── Legend ──────────────────────────────────────────────────── */}
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-[10px] text-white/30">
                <span className="flex items-center gap-1.5">
                    <span className="w-4 h-4 rounded border border-white/08 inline-flex items-center justify-center">
                ...
                <span className="flex items-center gap-1.5">
                    <span className="w-4 h-4 rounded border border-white/15 inline-block"/>
                    Available — click to allocate
                </span>
                <span className="flex items-center gap-1.5">
                    <span className="w-4 h-4 rounded inline-block" style={{ background: 'rgba(245,158,11,0.2)', border: '1px solid rgba(245,158,11,0.5)' }}/>
                    Selected / partial ranks
                </span>
                <span className="text-white/18">Right-click a node to remove one rank</span>
            </div>
        </div>
    )
}
