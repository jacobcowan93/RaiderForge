...
import { MAX_ACTIVE_TRIALS, MAX_IN_PROGRESS_STARTS_PER_WEEK } from '@/lib/progression/localProgressStore'
...
export function TrialDetailProgressToolbar({ trialId, title }: Props) {
    const { hydrated, getTrialStatus, setTrialStatus, activeTrialCount, inProgressStartsThisWeek } = useLearningProgress()
    ...
    const status = getTrialStatus(trialId)
    const atActiveCap = status !== 'in_progress' && activeTrialCount >= MAX_ACTIVE_TRIALS
    ...
    const apply = (next: LearningItemStatus) => {
        setError('')
        const r = setTrialStatus(trialId, next)
    ...
    return (
        <div
            ...
            role="region"
            aria-label={`Progress for trial ${title}`}
        >
            ...
            <div className="flex flex-wrap gap-2">
                <button
                    ...
                    aria-label={
                        atWeeklyCap
                            ...
                            : atActiveCap
                              ? `Cannot mark active: already ${MAX_ACTIVE_TRIALS} trials active`
                              : `Mark trial ${title} as in progress`
                    }
                ...
                <button
                    ...
                    aria-label={`Mark trial ${title} as completed`}
                ...
                <button
                    ...
                    aria-label={`Clear progress for trial ${title}`}
                >
                    Clear
                </button>
            </div>
            {atActiveCap ? (
                <p className="text-[11px] text-amber-200/70">
                    Active trial cap reached ({MAX_ACTIVE_TRIALS}). Complete or clear another trial first.
                </p>
            ) : null}
